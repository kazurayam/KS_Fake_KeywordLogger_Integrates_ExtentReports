...
**The Test Case `TC1` should remain the same as before. Test Cases should not make any immediate call to the Extent Reports API. The ordinary `WebUI.comment(String message)` should redirect the message into the new report generated by Extent Reports as well.**
