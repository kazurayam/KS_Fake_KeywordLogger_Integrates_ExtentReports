# A fake KeywordLogger class integrates the Extent Reports framework into Katalon project nicely
...
Also I made 2 Test Cases `TC1` and `` TC2` ``:
...
When I exected the `TS1`, I could see an built-in HTML report was generated, like this:
...
Every Katalon users will find there is nothing special in TS1, TC1, TC2 and builtin HTML report. It’s a boring stuff.

Now I want to add another format of test execution report using [Extent Reports](https://extentreports.com/). The report would like something like this:

![TS1 ExtentReports](https://kazurayam.github.io/KS_Fake_KeywordLogger_Integrates_ExtentReports/images/TS1_ExtentReports.png)

Now I would set a constraint in achieving the Extent Reports integration into Katalon project.

**The Test Case `TC1` should remain the same as before. It should make no immediate call to the Extetn Reports API. The ordinary `WebUI.comment(String message)` should write the message into the HTML generated by Extent Reports as well.**
