# A fake KeywordLogger class nicely integrates the Extent Reports into Katalon project
...
Now I want to add another format of test execution report which is generated by [Extent Reports](https://extentreports.com/). The report would like something like this:

![TS1 ExtentReports](https://kazurayam.github.io/KS_Fake_KeywordLogger_Integrates_ExtentReports/images/TS1_ExtentReports.png)

Now I would set a constraint in achieving the Extent Reports integration into Katalon project. **The Test Case `TC1` should remain the same as before. It should make no immediate call to the Extent Reports API. The ordinary `WebUI.comment(String message)` should redirect the message into the new report as well.**
